...
export default class News extends Component {
  ...
  async updateNews() {
    try {
      ...
      const res = await fetch(
        `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=d40405e197544ec791c7eef6f9ad79ba&page=${this.state.page}&pageSize=${this.props.pageSize}`
      );
      try {
  ...
  fetchMoreData = async () => {
    ...
    try {
      const res = await fetch(
        `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=71261cb6a60b4990ab061ba6643fdb0f&page=${this.state.page}&pageSize=${this.props.pageSize}`
      );
      // try {
      //   this.setState({ loading: true });
      // }
    ...
    return (
      <>
        <div className="text-center" style={{ marginTop: "90px" }}>
          <h2>{`Newsifyy - Top ${this.capitalizeFirstLetter(
            this.props.category
          )} Headlines`}</h2>
